...
def build_time_regexps():
    ...
    if not TIME_REGEX_CACHE:
        ...
        add_re(r"T?(?P<hour>[0-9]{2}):" r"(?P<minute>[0-9]{2}):" r"(?P<second>[0-9]{2}" r"([,.][0-9]+)?)")
...
def parse_time(timestring):
    ...
    for pattern in isotimes:
        ...
        if match:
            ...
            if "second" in groups:
                second = Decimal(groups["second"]).quantize(Decimal(".000001"), rounding=ROUND_FLOOR)
            ...
            if "minute" in groups:
                ...
                second = Decimal((minute - int(minute)) * 60).quantize(Decimal(".000001"), rounding=ROUND_FLOOR)
                microsecond = (second - int(second)) * int(1e6)
                return time(
                    int(groups["hour"]),
                    int(minute),
